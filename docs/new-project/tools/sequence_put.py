...
Client                              Server
 ...
 4. Put{size=len-offset, offset,
        ...
        plaintext_size, no_clobber} ─▶  5. ACL(write) / resolve_parent / temp 名拒否
                                     6. 検証必須形: offset&gt;0 or Zstd → checksum 必須
                                     7. Zstd: size == plaintext_size
                                     8. offset+size ≤ max_file_size
                                     9. no_clobber → lstat 既存なら AlreadyExists
                                    10. UploadClaim(final) else AlreadyExists
...
<tr><td>Trailer</td><td>32 バイト揃う → Verify</td></tr>
...
"steps.transaction-boundary":"<p>コミット(rename)が単一の原子点。コミット前の失敗は final に影響しない。<code>no_clobber</code> は開始時と コミット直前の 2 回検査するが、rename 自体は上書きになるため、最終的な競合は「同一宛先の claim」で排他する。</p>",
...
"exceptional-flows.timeout-retry":"<ul><li>要求単位のタイムアウトなし(QUIC アイドルに委ねる)。サーバは受信が 30 s 止まったストリームを接続ごと切る(アイドル)。</li><li>再送は Unsupported / StalePartial とも各 1 回。</li></ul>",
"exceptional-flows.compensation":"<p>失敗分類ごとの temp / 会計規則は転送エンジン設計書 §3.4 の表が正本。返金は「再開プレフィクス分」と「予約分」の 2 種類。</p>",
...
"non-functional.audit":"<p>開始(ユーザ、パス、offset、size、encoding、no_clobber)、結果(理由、書込バイト数、所要時間)、コミット成功をログ。メトリクス: <code>uploads_completed_total</code>、<code>bytes_received_total</code>(平文)、<code>requests_failed_total</code>。</p>",
