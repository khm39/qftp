...
"diagram.diagram-summary":"<p>上のテキスト図は、retry を要求するサーバに対する初回接続(チケットなし)と、チケットありで early data を送った場合の両方を 1 本にまとめています。early data 中の要求は許可リスト(Cd / Pwd / Stat / Quit)かつ identity gate 非該当のときのみ実行され、それ以外は <code>Unsupported</code> で拒否されます(early data を送るクライアントはハンドシェイク後に即時再送する)。参照クライアントは early data にアプリケーションデータを載せないため、図の early data 経路はサーバ側の受理能力を示すもので、e2e ではテスト用フックから送ります。</p>",
"exceptional-flows.failure-cases":"""<ul><li>ALPN 不一致: ハンドシェイク失敗。クライアントは「サーバはこのバージョンを話さない」と報告(69)。</li><li>token 不正 / 期限切れ: 黙って破棄。クライアントは次の Initial で新しい Retry を受ける。</li><li>接続上限: 破棄 + <code>qftp_connections_rejected_caps_total</code>。</li><li>mTLS 必須で cert なし、未知 CN、複数ユーザ一致: close <code>0x101</code>。クライアントは 77。</li><li>TOFU 不一致: クライアントが close、77。</li><li>チケット拒否(サーバ側 0-RTT 不可): quiche が 1-RTT にフォールバック。チケットは破棄して再取得。</li><li>early data 中の不許可要求: <code>Unsupported</code>(送った側は即時再送)。</li></ul>""",
...
"non-functional.concurrency":"<ul><li>接続スロットとレートバケットはイベントループ単一スレッドで扱い、ロックなし。</li><li>identity 昇格中に到着した要求は、昇格後のユーザで再評価する(昇格前に <code>spawn_blocking</code> に投げた結果は破棄して <code>Unsupported</code> にする)。</li></ul>",
