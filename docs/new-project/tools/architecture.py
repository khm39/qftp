...
ANSWERS={
"overview.name":"<p>qftp(QUIC File Transfer Protocol)リファレンス実装 第 2 世代。ネイティブサーバ、ネイティブクライアント、管理 CLI、および(後続区分の)ブラウザ向け WebTransport ブリッジを含みます。</p>",
...
"overview.positioning":"<p>本設計パッケージの最上位文書です。下位に機能設計書(転送エンジン / サーバ / クライアント / 管理 CLI / Web ブリッジ)、シーケンス設計書(接続確立 / Get / Put)、運用設計書、画面設計書(Web クライアント)が並びます。上位にあたる要件は <code>00-background/decisions.md</code>(ADR)と本書 §2 の課題・成功指標で表します。管理 CLI と画面については設計書を持たず、参照文書(<code>40-reference/</code>)で規定します。</p>",
...
<p>conformance は <code>qftp-wire</code> の tests / examples、フィクスチャ・e2e・ベンチは <code>qftp-server</code> の test-util feature / tests / benches に置き、独立クレートにしない(ADR-007)。境界の根拠は <code>30-repository/repository-layout.md</code> を参照。</p>""",
...
"technology-stack.constraints":"<ul><li>ワイヤプロトコル <code>qftp/1</code> は凍結済みで変更しない。</li><li>QUIC スタックは quiche を基本とする(方針)。</li><li>quiche は BoringSSL の C/C++ ビルドを要するため、クロスビルド(aarch64、musl)は CI で常時検証する。</li><li>Web ブリッジは WebTransport を要し、quiche では終端できない(調査済み、ADR-006)。wtransport(quinn + tokio)を別バイナリで使う。</li><li>OS 対応は Linux / macOS。Windows は 対象外で、非 Unix では起動時にエラー終了する(ADR-011)。</li></ul>",
"quality-attributes.performance":"<ul><li>単一接続の Get / Put スループットが 1 Gbps 級のリンクを飽和させること(プロトタイプの流量制御窓 16 MiB / 64 MiB を維持)。</li><li>ディスク I/O がイベントループをブロックしない(1 つの遅い転送が他接続の Ls 応答時間に影響しない)。</li><li>目標数値はベンチ(乱数ペイロード、圧縮あり / なし)で ベンチで確定する。</li></ul>",
...
"quality-attributes.scalability":"<ul><li>コア区分 は単一イベントループ。CPU スケールアウトは複数プロセス(別ポートまたは UDP LB)で行い、プロセス内マルチループは QUIC の CID ルーティング要件があるため採らない。</li><li>ディスク I/O は tokio のブロッキングプール(<code>max_blocking_threads</code> を設定で指定、既定は CPU 数 × 2)。</li><li>接続数は <code>max_connections</code> / <code>max_connections_per_ip</code> で明示的に上限を置く。</li></ul>",
...
"risks.mitigations":"<ul><li>実装前に quiche の 0-RTT 受理 + 1-RTT ゲート、retry、GSO を最小サーバで 技術検証を行い、結果を ADR に追記する。</li><li>エンジンの API を先に固定し、純メモリのホストでユニットテストを書いてからサーバに組み込む。</li><li>e2e テスト仕様(<code>40-reference/e2e-test-spec.html</code>)を受け入れ条件とする。</li><li>CI に MSRV ジョブと aarch64 クロスビルドを最初から置く。</li></ul>",
"references.related-docs":"<ul><li>機能設計書: <a href=\"feature-transfer-engine.html\">転送エンジン</a>、<a href=\"feature-server.html\">サーバ</a>、<a href=\"feature-client.html\">クライアント</a>、<a href=\"feature-web-bridge.html\">Web ブリッジ</a>(管理 CLI は <a href=\"../40-reference/cli-reference.html#admin\">CLI リファレンス</a> と <a href=\"../40-reference/file-formats.html\">ファイル形式リファレンス</a> で規定)</li><li>シーケンス設計書: <a href=\"sequence-connection-setup.html\">接続確立</a>、<a href=\"sequence-get-transfer.html\">Get</a>、<a href=\"sequence-put-transfer.html\">Put</a></li><li><a href=\"operations-qftp-server.html\">運用設計書</a>(画面設計書は Web 区分の着手時に作成)</li><li>参照文書: <code>40-reference/</code>(転送エンジン API、設定、CLI、ファイル形式、e2e テスト)</li><li>決定事項: <code>00-background/decisions.md</code>(ADR-001〜013)</li><li>リポジトリ構成: <code>30-repository/repository-layout.md</code></li></ul>",
