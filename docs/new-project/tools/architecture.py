...
ANSWERS={
"overview.name":"<p>qftp(QUIC File Transfer Protocol)リファレンス実装 第 2 世代。ネイティブサーバ、ネイティブクライアント、管理 CLI、および(後続フェーズの)ブラウザ向け WebTransport ブリッジを含みます。</p>",
"overview.purpose":"<p>プロトタイプ実装を捨て、凍結済みワイヤプロトコル <code>qftp/1</code> に準拠した実装を新規リポジトリで起こすための全体設計です。対象はシステム全体(全バイナリと共有ライブラリ)で、ワイヤプロトコル自体は本書の対象外です(<a href=\"../10-protocol/README.md\">プロトコル仕様</a>を正本とします)。</p>",
"overview.audience":"<p>実装を担当する開発者、レビュアー、および運用設計を行う担当者。</p>",
"overview.positioning":"<p>本設計パッケージの最上位文書です。下位に機能設計書(転送エンジン / サーバ / クライアント / 管理 CLI / Web ブリッジ)、シーケンス設計書(接続確立 / Get / Put)、運用設計書、画面設計書(Web クライアント)が並びます。上位にあたる要件定義は <code>00-background/prototype-assessment.md</code>(プロトタイプの評価)と <code>00-background/decisions.md</code>(決定事項)で代替しています。</p>",
...
"system-overview.context-diagram":"""<pre>
 ...
 (Phase 7) [ブラウザ SPA] ── WebTransport(HTTP/3) ──▶ [qftp-web-bridge] ──▶ 同じルート / users.toml
...
<tr><td><code>qftp-core</code></td><td>パスサンドボックス、ユーザ / ACL / クォータ、メタデータ操作(Ls ページング等)、<b>sans-I/O 転送エンジン</b>(Get / Put の状態機械)、zstd、temp 名規則、X.509 からの identity 抽出</td><td>wire</td></tr>
...
<tr><td><code>qftp-client</code>(bin)</td><td>CLI、REPL(パーサ / コマンド / 補完)、one-shot、出力整形と終了コード。Phase 6: sync / watch</td><td>client-core</td></tr>
...
<tr><td><code>qftp-web-bridge</code>(Phase 7)</td><td>WebTransport 終端(wtransport、ADR-006)+ 同じ転送エンジンの tokio ホスト</td><td>core</td></tr></table>
<p>当初 12 クレートだった案から、<code>qftp-conformance</code>(→ wire の tests / examples)、<code>qftp-testkit</code> / <code>qftp-bench</code> / <code>qftp-e2e</code>(→ server の test-util feature / benches / tests)を統合し、MVP 時点で 7 クレート + fuzz とした。境界の根拠は <code>30-plan/repository-layout.md</code> を参照。</p>""",
...
"system-overview.data-flow":"""<ol><li>クライアントが QUIC 接続を確立(retry → ハンドシェイク → identity 解決)。</li><li>コマンドごとに双方向ストリームを 1 本開き、<code>Request</code> フレームを送る。</li><li>サーバはフレームを <code>qftp-wire</code> でデコードし、レート制限・0-RTT ゲート・ACL を通したうえで <code>qftp-core</code> に渡す。</li><li>メタデータ操作は tokio のブロッキングプール(<code>spawn_blocking</code>)で実行され、<code>Response</code> がストリームに返る。</li><li>Get / Put は sans-I/O エンジンがストリーム上のバイト列とブロッキングプールのファイル操作を仲介し、BLAKE3 トレーラで整合性を検証する。</li><li>メトリクスはコネクションスレッドがカウンタを更新し、HTTP スレッドが読み出す。</li></ol>""",
...
<tr><td>転送エンジンを async fn で書く</td><td>ネイティブ側が同期ループのため使えない。sans-I/O なら両方から駆動できる</td></tr>
<tr><td>OS ユーザ分離(setuid ワーカー)</td><td>プロトタイプで実装後に却下済み。root 相当コンポーネントの追加と仮想ユーザ方針の矛盾(ADR-004 として引き継ぐ)</td></tr></table>""",
"technology-stack.constraints":"<ul><li>ワイヤプロトコル <code>qftp/1</code> は凍結済みで変更しない。</li><li>QUIC スタックは quiche を基本とする(方針)。</li><li>quiche は BoringSSL の C/C++ ビルドを要するため、クロスビルド(aarch64、musl)は CI で常時検証する。</li><li>Web ブリッジは WebTransport を要し、quiche では終端できない(調査済み、ADR-006)。wtransport(quinn + tokio)を別バイナリで使う。</li><li>OS 対応は Linux / macOS。Windows は Phase 5 まで対象外で、非 Unix では起動時にエラー終了する(ADR-011)。</li></ul>",
"quality-attributes.performance":"<ul><li>単一接続の Get / Put スループットが 1 Gbps 級のリンクを飽和させること(プロトタイプの流量制御窓 16 MiB / 64 MiB を維持)。</li><li>ディスク I/O がイベントループをブロックしない(1 つの遅い転送が他接続の Ls 応答時間に影響しない)。</li><li>目標数値はベンチ(乱数ペイロード、圧縮あり / なし)で Phase 5 に確定する。</li></ul>",
...
"quality-attributes.scalability":"<ul><li>MVP は単一イベントループ。CPU スケールアウトは複数プロセス(別ポートまたは UDP LB)で行い、プロセス内マルチループは QUIC の CID ルーティング要件があるため採らない。</li><li>ディスク I/O は tokio のブロッキングプール(<code>max_blocking_threads</code> を設定で指定、既定は CPU 数 × 2)。</li><li>接続数は <code>max_connections</code> / <code>max_connections_per_ip</code> で明示的に上限を置く。</li></ul>",
...
<tr><td>ADR-011</td><td>Windows は Phase 5 まで対象外(非 Unix は起動時エラー)</td></tr>
...
"decisions.related-adrs":"<p><code>00-background/decisions.md</code>(ADR-001〜006)。プロトタイプ時代の ADR-0001(quiche 継続)と ADR-0002(OS ユーザ分離却下)は同ファイルに要約を引き継いでいます。</p>",
"risks.known-risks":"""<ul><li>quiche で TOFU / ホスト名検証がハンドシェイク後にしか行えないため、検査完了前にアプリケーションデータを送らない規律をクライアントコアで強制する必要がある。</li><li>sans-I/O エンジンとホストの境界設計を誤ると、ホスト側に状態が漏れてプロトタイプの二の舞になる。エンジンの公開 API を機能設計書(転送エンジン)で固定する。</li><li>BoringSSL のビルド時間と C++ ツールチェーン依存。</li><li>プロトタイプの細かな挙動(自動再開、StalePartial 再試行など)の取りこぼし。</li></ul>""",
"risks.open-questions":"<ul><li>可用性 SLO と性能目標の数値。</li><li>Phase 0 spike の結果(tokio current_thread 上の quiche 駆動構造、GSO)。</li></ul>",
"risks.mitigations":"<ul><li>Phase 0 で quiche の 0-RTT 受理 + 1-RTT ゲート、retry、GSO を最小サーバで spike し、結果を ADR に追記する。</li><li>エンジンの API を先に固定し、純メモリのホストでユニットテストを書いてからサーバに組み込む。</li><li>プロトタイプ評価文書の挙動一覧を e2e の受け入れ条件に含める。</li><li>CI に MSRV ジョブと aarch64 クロスビルドを最初から置く。</li></ul>",
"references.related-docs":"<ul><li>機能設計書: <a href=\"feature-transfer-engine.html\">転送エンジン</a>、<a href=\"feature-server.html\">サーバ</a>、<a href=\"feature-client.html\">クライアント</a>、<a href=\"feature-web-bridge.html\">Web ブリッジ</a>(管理 CLI は <a href=\"../40-reference/cli-reference.html#admin\">CLI リファレンス</a> と <a href=\"../40-reference/file-formats.html\">ファイル形式リファレンス</a> で規定)</li><li>シーケンス設計書: <a href=\"sequence-connection-setup.html\">接続確立</a>、<a href=\"sequence-get-transfer.html\">Get</a>、<a href=\"sequence-put-transfer.html\">Put</a></li><li><a href=\"operations-qftp-server.html\">運用設計書</a>(画面設計書は Phase 7 開始時に作成)</li><li>参照文書: <code>40-reference/</code>(転送エンジン API、設定、CLI、ファイル形式、e2e テスト)</li><li>背景: <code>00-background/prototype-assessment.md</code>、<code>00-background/decisions.md</code></li><li>計画: <code>30-plan/roadmap.md</code>、<code>30-plan/repository-layout.md</code></li></ul>",
