...
12. ◀── trailer(digest_len = 32 B) + FIN ──  12. Send(trailer, fin)
13. hash == trailer ? 確定 : ローカル削除 (65)
</pre>""",
"steps.sync-async":"<ul><li>クライアントは要求を送ったあと、FileReady・本体・トレーラを 1 ストリーム上で順次受ける(非同期到着、エンジンは <code>Event::Bytes</code> で処理)。</li><li>サーバのファイル読み取りはブロッキングプールへの非同期要求。</li><li>クライアントのローカル書込は同期(単一要求のため HOL は問題にならない)。</li></ul>",
...
<tr><td>任意</td><td>ReadFailed → Failed(<code>ResetStream 0x51</code>)。Cancel → Done(Cancelled、本体送出後なら <code>ResetStream 0x53</code>)</td></tr></table>
<p>クライアント側は Requesting → Header → Prefix → Body → Trailer → Verified / Discarded。</p>""",
"steps.transaction-boundary":"<p>クライアントのローカルファイルは、トレーラ検証成功までは「未確定」。検証失敗(トレーラ不一致・短い本体・復号失敗)では削除する。切断・取り消しでは受信済み分を残し、次回の get が再開する。</p>",
...
"non-functional.audit":"<p>転送開始(ユーザ、パス、offset、encoding)、完了 / 失敗(理由、バイト数、所要時間)をログ。メトリクス: <code>qftp_downloads_completed_total</code>、<code>qftp_bytes_sent_total</code>(ワイヤバイト、トレーラ込み)、<code>qftp_transfer_duration_seconds</code>。</p>",
