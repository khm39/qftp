...
| 4 | 既定ポリシー | **決定: default-on + 既圧縮データ自動回避**(拡張子/magic or 先頭ブロック圧縮率閾値で Identity 縮退)。**実装済(Phase 3a)**: 拡張子ヒューリスティク(`is_likely_incompressible`, rsync `--skip-compress`相当)で client Put / server Get が Identity 縮退、client `--no-compress` で全圧縮を無効化。 | ユーザー決定(2026-06-01): ログ/JSON/ソース中心の用途で帯域/転送時間の削減を取る。既圧縮メディア(jpeg/zip/mp4)は自動回避で逆効果を防ぐ。攻撃表面はファイル本体のみ・per-file独立フレームで限定的(§5)。**送信側ローカル挙動でワイヤ互換には無関係**(test-vectors不変で実証)。 |
